    ...
    def loaddata(self):
        for rowindex, dataseries in self.df.iterrows():  #recorro cada linea del dataframe (leer sobre iterables)
            if dataseries[0] in self.nodos.keys() and dataseries[1] in self.nodos.keys():
                self.nodos.get(dataseries[0]).agregarrelacion((dataseries[1], dataseries[2]))
                self.nodos.get(dataseries[1]).agregarrelacion((dataseries[0], dataseries[2]))
                ...
                self.nodos[nuevonodo.nombre] = nuevonodo
                self.nodos.get(dataseries[0]).agregarrelacion((dataseries[1], dataseries[2]))
                ...
                self.nodos[nuevonodo1.nombre] = nuevonodo1
                self.nodos[nuevonodo2.nombre] = nuevonodo2
