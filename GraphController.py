...
class Graph:
    ...
    def agregarnodos(self):              #funcion para agregar los nodos a partir de los datos
        for key in self.diccionario:     #bucle foreach para cada key del diccionario
            ...
            for relacion in self.diccionario[key].relaciones:   #por cada relacion de cada llave (for anidado)
    ...
    def mostrargrafo(self):
        ...
        plt.savefig('grafo.png')  #muestro el grafico en pantalla
        plt.close()

    def hallarcamino(self, alumno1, alumno2):
        if alumno1 not in self.g.nodes or alumno2 not in self.g.nodes:
            tkinter.messagebox.showinfo("Nodo no encontrado", "Uno de los nodos no se encuentra en el grafo")
        else:
